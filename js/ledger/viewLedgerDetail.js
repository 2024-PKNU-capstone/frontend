  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.code === 200) {
        ...
        const receiptId = response.data.receiptId; // 영수증 ID 추출
        if (receiptId) {
          loadReceiptUrl(receiptId); // 영수증 URL 요청
        }
      }
    },
    error: function (xhr) {
...
// 영수증 URL 요청 함수
function loadReceiptUrl(receiptId) {
  $.ajax({
    url: `${API_BASE_URL}/api/receipt/url?receiptId=${receiptId}`,
    type: 'GET',
    headers: {
      Authorization: accessToken,
    },
    success: function (response) {
      if (response.code === 200) {
        const receiptUrl = response.data.url; // 응답에서 영수증 URL 추출
        displayReceiptUrl(receiptUrl);
      }
    },
    error: function (xhr) {
      alert('영수증 데이터를 불러오는데 실패했습니다: ' + xhr.responseText);
    },
  });
}
...
// 영수증 URL을 HTML에 표시하는 함수
function displayReceiptUrl(url) {
  const receiptDetail = $('.receipt-detail');
  receiptDetail.html(`
    <img src="${url}" alt="영수증 이미지" style="max-width: 100%; height: auto;">
  `);
}
